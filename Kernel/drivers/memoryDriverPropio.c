...
//  PROVISORIA ESTA MACRO 
#define TOTAL_HEAP_MEMORY 1
...
void consult(char* buf){
    ...
    long freeMemory=0;
    long occupiedMemory;
    for(currentBlock=freep; currentBlock->s.ptr!=NULL;currentBlock=currentBlock->s.ptr){
        freeMemory+=currentBlock->s.size ;
    }

    long end =(long) sbrk(0); //current location of the program break
    
    occupiedMemory= end - freeMemory;
    
    copyAnswer(infoTotalMessage,end,buf);
    copyAnswer(infoLibreMessage,freeMemory,buf);
    copyAnswer(infoOcupadaMessage,occupiedMemory,buf);
    *(buf++)='\0';
}

static void copyAnswer(char* phrase,long memoryNum,char* buf){
    while(*phrase!='\0'){
        *(buf++)=*(phrase++);
    }
    buf+=numToStr(memoryNum,buf);
}
